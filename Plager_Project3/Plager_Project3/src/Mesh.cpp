...
void Mesh::draw(mat4 viewMat, mat4 projMat, vec3 light1Pos, vec3 light2Pos, vec3 eyePos, float time) {
	...
	mat4 m = translate(mat4(1.0), objPos);			
	modelMat = scale(m, objScale);
...
void Mesh::setPos(vec3 newPos)
{
	objPos = newPos; 
}

void Mesh::setScale(double newScale)
{
	objScale = vec3(newScale); 
}

void Mesh::setFillMode(GLenum newMode)
{
	fillMode = newMode; 
}
