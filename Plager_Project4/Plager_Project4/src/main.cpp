...
vec3 g_rayOrigin = vec3(0.0f, 5.0f, 20.0f);

vec3 g_wireSpherePos = vec3(0.0f, 5.0f, -8.0f);
float g_wireSphereSpeed = 0.5f; 
...
void display()
{
	...
	string resString = "Resolution: " + std::to_string(g_resWidth) + 'x' + std::to_string(g_resHeight);
	g_text.draw(10, 50, const_cast<char*>(resString.c_str()), g_winWidth, g_winHeight);

	string particleString = "# of Particles ((w + 1) x (h + 1)): " + std::to_string((g_resWidth + 1) * (g_resHeight + 1));
	g_text.draw(10, 70, const_cast<char*>(particleString.c_str()), g_winWidth, g_winHeight);
	...
	for (int i = 0; i < 4; i++) {
		glBegin(GL_LINES);
		glVertex3f(g_rayOrigin.x, g_rayOrigin.y, g_rayOrigin.z);
		glVertex3f(i < 2 ? -10.0f : 10.0f, i % 2 == 0 ? 10.0f : 0.0f, -5.0f); 
		glEnd(); 
	}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 45: 
		...
		break;
	case 'w': 
		g_wireSpherePos.z -= g_wireSphereSpeed;
		break; 
	case 's': 
		g_wireSpherePos.z += g_wireSphereSpeed;
		break; 
	case 'a': 
		g_wireSpherePos.x -= g_wireSphereSpeed;
		break; 
	case 'd': 
		g_wireSpherePos.x += g_wireSphereSpeed;
		break; 
	case 'u': 
		g_wireSpherePos.y += g_wireSphereSpeed;
		break;
	case 'j': 
		g_wireSpherePos.y -= g_wireSphereSpeed;
		break;
